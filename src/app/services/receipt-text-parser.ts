...
import { ExtractedReceiptLineItem, ParsedReceiptOcr } from '../models/extracted-receipt.model';
...
export function parseReceiptLineItemsText(text: string): ParsedReceiptOcr {
  ...
  for (const line of lines) {
    ...
    lineItems.push({ ...parsed, selected: true });
  }

  if (lineItems.length === 0) {
    lineItems.push(...parseReceiptFallbackLines(text));
  }
...
export function parseShoppingListText(text: string): ExtractedShoppingItem[] {
  const structured = parseShoppingListStructured(text);
  return structured.length > 0 ? structured : parseShoppingListFallback(text);
}
...
function parseReceiptFallbackLines(text: string): ExtractedReceiptLineItem[] {
  const items: ExtractedReceiptLineItem[] = [];

  for (const rawLine of text.split('\n')) {
    const line = rawLine.replace(/\s+/g, ' ').trim();
    if (line.length < 3 || RECEIPT_SKIP_LINE.test(line)) {
      continue;
    }

    const parsed = parseReceiptLine(line);
    if (parsed) {
      items.push({ ...parsed, selected: true });
      continue;
    }

    if (/[a-zA-Z]{3,}/.test(line) && !/^\d+$/.test(line)) {
      items.push({
        description: capitalizeWords(line.replace(BULLET_PREFIX, '')),
        quantity: 1,
        amountKes: 0,
        selected: true,
      });
...
function parseShoppingListStructured(text: string): ExtractedShoppingItem[] {
  const seen = new Set<string>();
  ...
  for (const rawLine of text.split('\n')) {
    ...
    if (name.length < 2 || /^\d+$/.test(name) || /^\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}$/.test(name)) {
      continue;
    }
...
function parseShoppingListFallback(text: string): ExtractedShoppingItem[] {
  const seen = new Set<string>();
  const items: ExtractedShoppingItem[] = [];
  ...
  for (const rawLine of text.split('\n')) {
    ...
    if (line.length < 3 || SKIP_LINE.test(line) || RECEIPT_SKIP_LINE.test(line)) {
      continue;
    }
